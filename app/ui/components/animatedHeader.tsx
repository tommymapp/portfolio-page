...
export default function AnimatedHeader({ children, variant: type = 'default' }: {
	...
	return (
        <header className={`min-h-[480px] ${type == 'hero' ? 'h-[80vh]' : 'h-[480px]'}`}>
            {children}
            <div className={`animated-header absolute w-full overflow-hidden top-0 left-0 right-0 bottom-0 z-[-1] min-h-[480px] ${type == 'hero' ? 'h-[80vh]' : 'h-[480px]'} box-content -mt-[116px] pb-[116px]`} ref={animRef}></div>
        </header>
    )
}